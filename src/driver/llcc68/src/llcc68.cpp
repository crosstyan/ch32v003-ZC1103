...
int16_t LLCC68::begin(uint8_t cr, uint8_t syncWord, uint16_t preambleLength, float tcxoVoltage, bool useRegulatorLDO) {
  ...
  this->bandwidthKhz    = fpm::fixed_16_16 {500.0};
...
int16_t LLCC68::startReceive(uint32_t timeout, uint16_t irqFlags, uint16_t irqMask, size_t len) {
  (void)len;
...
int16_t LLCC68::startReceiveDutyCycleAuto(uint16_t senderPreambleLength, uint16_t minSymbols, uint16_t irqFlags, uint16_t irqMask) {
  ...
  uint32_t symbolLength = (static_cast<uint32_t>(10 * 1000) << this->spreadingFactor) / static_cast<uint32_t>(fpm::fixed_16_16 {10} * this->bandwidthKhz);
...
int16_t LLCC68::startReceiveCommon(uint32_t timeout, uint16_t irqFlags, uint16_t irqMask) {
  // set DIO mapping
...
int16_t LLCC68::setBandwidth(float bw) {
  ...
  auto fixed_bw = fpm::fixed_16_16{bw};
  // check allowed bandwidth values
  auto bw_div2 = static_cast<uint8_t>(fixed_bw / 2 + fpm::fixed_16_16{0.01});
  switch (bw_div2) {
    case 3: // 7.8:
